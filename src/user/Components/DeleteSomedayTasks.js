...
class DeleteSomedayTasks extends Component {
    ...
    deleteSomedayTask(item) {
        ...
        return <div className="SomedayDeleteTaskName" key={item} ><div className="deleteSomeday"><svg onClick={() => this.delete(item)} width="24" height="24" viewBox="0 0 24 24"><g fill="none" fill-rule="evenodd">
    ...
    delete(key) {
      ...
      render() {
       ...
       if (this.props.show) {
         ...
         var taskItems = taskEntries.map(this.deleteSomedayTask);
         showDeleteHideClassName = <div >
         {taskItems}
        </div>;
       } 
       return (
         <div className="SomedayDeleteTaskList">{showDeleteHideClassName}</div>
       );
     }
}
