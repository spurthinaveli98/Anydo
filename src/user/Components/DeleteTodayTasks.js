...
class DeleteTodayTasks extends Component {
    ...
    deleteTodayTask(item) {
        ...
        return <div className="TodayDeleteTaskName" key={item} ><div className="deleteToday"><svg onClick={() => this.delete(item)} width="24" height="24" viewBox="0 0 24 24"><g fill="none" fill-rule="evenodd">
    ...
    delete(key) {
      ...
      render() {
       ...
       if (this.props.show) {
         ...
         var taskItems = taskEntries.map(this.deleteTodayTask);
         showDeleteHideClassName = <div >
         {taskItems}
        </div>;
       } 
       return (
         <div className="TodayDeleteTaskList">{showDeleteHideClassName}</div>
       );
     }
}
